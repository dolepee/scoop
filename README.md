...
- Live feed snapshot, as of `2026-06-12T15:35:15.592Z`: 25 cycles, 22 paid cycles, 16 `NO_TRADE` cycles, 9 proposed `TRADE` cycles, 0 executed trades, equity `16.36`, floor `13.73`, chain verification `true`. <!-- REFRESH BEFORE SUBMIT -->
...
| Max drawdown risk gate | The ratchet governor stands down well before the competition DQ line; the current floor is `13.73` as of `2026-06-12T15:35:15.592Z`. <!-- REFRESH BEFORE SUBMIT --> |
| Non-zero in-scope balance | Each feed cycle carries `inScopeUsd`; latest value is `16.36`, warning flag `false`, as of `2026-06-12T15:35:15.592Z`. <!-- REFRESH BEFORE SUBMIT --> |
...
GitHub Actions runs `.github/workflows/scoop-cycle.yml` every two hours, verifies receipts, rebuilds the feed, and commits new receipts. A local Windows dispatcher, if used, should call the same cycle path and must not bypass `npm run receipts:verify`.

## Phases

| Date | Phase | Status |
| --- | --- | --- |
| Jun 10-12, 2026 | Observation build | Current phase as of Jun 12: paid data, receipt chain, dashboard, and governor exercising in observe mode. |
| Jun 16-19, 2026 | Armed rehearsal | Planned real-money rehearsal at `$5` scale. |
...
Transparent build-phase note: commit `254bde6` rechained early receipts after fixing a JSON round-trip checksum asymmetry, before go-live. Policy from here: no rechain after live submission; bad receipts get corrected by a new receipt and a new commit, not by rewriting history.
